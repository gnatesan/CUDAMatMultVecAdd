...
// dimension variable is the footprint size
  ...
  for(int y=0; y<correct.height; y++) {
    for(int x=0; x<correct.width; x++) {
      ...
      if(fabs(it - M.elements[y*M.width+x])> epsilon*it) {
        if (M.width != correct.width) printf("Mismatching width");
        if (M.height != correct.height) printf("Mismatching height");
        printf("Row %d Col %d Correct Value %f Calculated Value %f\n", y, x, it, M.elements[y*M.width+x]);
        errCnt++;
